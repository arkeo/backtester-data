...
class Instrument:
    ...
    # First month the source actually has, "YYYY-MM". Only the generated crypto
    # pairs carry it, and only because there are thousands of them: without it
    # a coin listed last year would be asked for every month back to 2017 and
    # would collect eighty 404s before finding anything.
    first_month: str = ""
    last_month: str = ""
...
# --- every other pair the archive carries ----------------------------------
# Generated by installer/make_crypto_list.py. Three and a half thousand of
# them, so nothing here may cost anything per instrument: the rows are a plain
# tuple list, the price step is worked out from the data the first time a pair
# is downloaded rather than looked up anywhere, and nothing touches the disk.
try:
    from .crypto_pairs import PAIRS as _PAIRS
except ImportError:                             # not generated yet
    _PAIRS = []

_TAKEN = {i.symbol for i in INSTRUMENTS}

for _code, _base, _quote, _first, _last in _PAIRS:
    # A generated row must never shadow a hand-written one. The archive does
    # carry a few codes that collide with the names used above, and the later
    # entry silently won: gold's neighbour in the list would have been a
    # different market with no price step, and the collision showed up only as
    # an instrument that had been working suddenly not knowing its own scale.
    if _code in _TAKEN:
        continue
    _TAKEN.add(_code)
    INSTRUMENTS.append(Instrument(
        symbol=_code, name=f"{_base} / {_quote}", group="crypto",
        # Zero means "not known yet". Every pair quotes to its own number of
        # decimals and there is no listing that gives them, so it is measured
        # from the first month fetched and remembered from then on.
        point=0.0, digits=0,
        source="binance", code=_code, first_year=int(_first[:4]),
        contract=1.0, quote=_quote,
        first_month=_first, last_month=_last,
    ))
...
# --------------------------------------------------------------------------
# price steps worked out from the data
# --------------------------------------------------------------------------
#
# Bars are stored as integers scaled by `point`, so the step has to be settled
# before a single bar can be written — and for the generated pairs it is not
# known in advance. It is measured once, from the first month actually
# fetched, and kept here so every later run, and the rest of the application,
# agrees with what is already on disk.
...
def _steps_path() -> str:
    from . import paths
    return os.path.join(paths.data_dir(), "price-steps.json")


def load_steps() -> None:
    """Read the remembered steps. Called once at startup."""
    global _STEPS_FILE
    _STEPS_FILE = _steps_path()
    try:
        with open(_STEPS_FILE, "r", encoding="utf-8") as f:
            _STEPS.update({k: float(v) for k, v in json.load(f).items()})
    except (OSError, ValueError):
        pass


def remember_step(symbol: str, point: float) -> None:
    """Settle an instrument's price step for good."""
    symbol = symbol.upper()
    if _STEPS.get(symbol) == point:
        return
    _STEPS[symbol] = point
    path = _STEPS_FILE or _steps_path()
    try:
        ...
        pass                                    # in memory is still better than not


def get(symbol: str) -> Instrument:
    try:
        inst = BY_SYMBOL[symbol.upper()]
    except KeyError:
        raise KeyError(f"unknown instrument {symbol!r}") from None
    step = _STEPS.get(inst.symbol)
    if step and not inst.point:
        return dataclasses.replace(
            inst, point=step, digits=max(0, round(-math.log10(step))))
    return inst


def needs_step(symbol: str) -> bool:
    return not get(symbol).point
